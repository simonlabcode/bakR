...
object. To get a sense of what this looks like and the information
it contains, let's simulate one:

```{r}

# Simulate DynamicSeqData object with 100 features (e.g., genes)
sim_data <- sim_DynamicSeqData(100)

# Extract DynamicSeqData object from simluation output
DynData <- sim_data$DynData

```

The DynamicSeqData object, referred to as DynData in this example,
...
Lets get an idea for what the cB looks like 1st.

```{r}

# Extract 1st component of DynData, the cB dataframe
cB <- DynData$cB

# Observe contents of cB
# Some columns are added by the simulation and don't need to be observed
knitr::kable(head(cB[,c(1, 5, 7, 9, 8)]), "pipe")
...
defined (e.g., WT_rep1_2021_10_06), but in the simulated data it is simplified
to a numerical ID.
...
1. XF: Name of the feature (gene, exon, etc.) that the read comes from. Kinetic parameters will be estimated for each feature, so this should represent the functional unit who's metabolic kinetics you are interested in. Like the sample column, this is typically a character vector, but is a simple numerical ID in the simulated data.
...
1. n: Number of reads with identical data for the other 4 columns. For example, n = 5 in the 3rd row means that there are 5 reads from sample 1 that mapped to XF 1, had 0 T to C mutations, and had 36 Ts.
...
Now let's look at the metadf dataframe:

```{r}

# Extract 1st component of DynData, the cB data frame
metadf <- DynData$metadf

# Observe contents metadf data frame
knitr::kable(metadf, row.names = 1:nrow(metadf))
