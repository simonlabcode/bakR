...
that bakR makes use of under the hood for the heavier, higher performance
models, as well as the R package rstan to allow interfacing between Stan and R. The biggest challenge is getting a C++ compiler configured to work with your installation of R. Instructions [here](https://github.com/stan-dev/rstan/wiki/RStan-Getting-Started) will hopefully help with installation of rstan. 
...
Fit <- bakRFit(bakRData, StanRateEst = FALSE)
...
One of the most important messages is that regarding the estimated pnews and pold. The pnews are the estimated mutation rates of reads from new RNAs (new meaning RNAs synthesized after 4sU labeling) in each sample (muts = Exp_ID, and reps = a numerical replicate ID that corresponds to the order replicates appear in the cB), and pold is a global estimate of the background mutation rate used in all analyses. The simulated mutation rates are 0.05 for new reads and 0.001 for old reads in all samples this simulation, so the estimates are pretty close. "But Isaac, why are the pnews consistently underestimated"? Good question, its because this is a small simulated dataset with a somewhat tight distribution of fraction news. fast_analysis() makes lots of assumptions to assure maximum efficiency, and one is that the highest mutation rates in s4U fed samples come from features with almost 100% labeling (fraction new = 1). That won't be true in small simulated datasets, but it's still not that bad, everything considered. If you have better mutation rate estimates from elsewhere (say a run of the full Stan model implemented in bakR), you can pass these estimates to bakRFit():
...
An even better, though slightly less efficient solution is to use a more rigorous statistical model implemented in Stan to estimate mutation rates. The idea is to run this model on only a small subset of features with sufficient sequencing depth and a fraction new moderately close to 0.5 so that there is representation of both labeled and unlabeled reads. Note, this is the default mutation rate strategy in bakRFit:
...
No matter how you decide to run the fast model, you might wonder how well it worked. As mentioned earlier, one benefit of using simulated data is that you can easily answer that question by comparing the simulated parameters (e.g., the fraction of RNA that was synthesized during 4sU labeling, a.k.a. the fraction new) to the model's estimates:
...
In this case, replicates separate from eachother fairly well, which is promising and the difference between the red point in the lower left and those in the upper left is not significant enough to cause concern. I have found that clustering is improved with the full MCMC fit, so if you've taken the time to run that model, I suggest using it for any PCA plots.
...
1. I want to perform more complicated comparisons, not just a set of comparisons with respect to a single reference sample. How do I do that?
1. What if my population of cells are dynamically responding to a perturbation during my experiment? The steady-state relationship between the fraction new and degradation rate that bakR seems to assume won't hold. What should I do about that? 
...
If either of the first two questions are of interest to you, head over to the Further-Analyses vignette! 

If the final question is of interest, then I would suggest heading over to some of the amazing documentation written by the Stan team. In short though, warnings from Stan are a sign that the model did not converge. The cause of this is likely anomalies in your data. Some challenges that I have come across that could lead to such warnings include:
