...
The 1st step to using bakR is to create a DynamicSeqData
...
Since we've already run the fast_analysis once, we can speed up the analysis by passing the Fit object from the first run rather than the DynamicSeqData object. This allows bakR to skip the time consuming pre-processing steps. Just make sure to set FastRerun to TRUE. 
